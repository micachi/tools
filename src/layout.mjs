...
export const UI = {
  ja: {
    ...
    adLabel: "広告",
    footHome: "🧰 ツール一覧",
    footAmbient: "✦ 別サイト — <a href=\"https://ambient.wicachi.com/\">Ambient</a>：ずっと眺めていられる、ブラウザのスクリーンセーバー",
  ...
  en: {
    ...
    footHome: "🧰 All tools",
    footAmbient: "✦ Another site — <a href=\"https://ambient.wicachi.com/\">Ambient</a>: a browser screensaver you can watch forever",
...
// ── AdSense ───────────────────────────────────────────────
/**
 * 広告ポリシー（意図的に「手動・1枠・固定位置」で固定している）
 *
 *  1. 自動広告（Auto ads）は使わない。
 *     - `enable_page_level_ads` / `data-ad-layout-key` / オーバーレイ類は一切出力しない。
 *     - 自動広告が増えるとツールUIが広告に埋もれて用途を失うため。
 *     - 万一 AdSense 管理画面側で自動広告が ON になっても、下の test.js 第26節で
 *       コード側の回帰を必ず検知する。
 *  2. 1ページにつき広告ユニットはちょうど 1 個。
 *  3. 配置は `<!--@AD@-->` プレースホルダのある位置（無ければ本文末尾）。
 *     操作ボタンの直近に置かない（誤クリック誘発はポリシー違反リスク）。
 *  4. 「広告」ラベル必須（Google ポリシー）。
 *
 * ※ 自動広告の実効的な制御は AdSense 管理画面でのみ可能:
 *    自動広告 OFF / 除外エリア / 除外ページ（support.google.com/adsense/answer/9305577）
 *    CSS で配信済み広告を隠すのはポリシー違反になり得るため行わない。
 */
